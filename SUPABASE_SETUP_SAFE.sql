-- SAFE SUPABASE SETUP FOR USER MANAGEMENT
...
-- Step 6: Create or replace functions
...
-- Step 7: Create triggers (drop first if they exist)
...
-- Step 10: Insert/update current user profile
DO $block$
DECLARE
...
BEGIN
    ...
    ELSE
        RAISE NOTICE 'No users found in auth.users table. Make sure you are logged in.';
    END IF;
END $block$;

-- Step 11: Create sample notifications (safely)
DO $block$
DECLARE
    user_count INTEGER;
BEGIN
...
BEGIN
    ...
    SELECT COUNT(*) INTO policy_count FROM pg_policies WHERE tablename IN ('profiles', 'notifications') AND schemaname = 'public';
    ...
    ELSE
        RAISE NOTICE 'Setup successful! Your application should now work properly.';
    END IF;
END $block$;
...
-- Success message
SELECT 'Database setup completed successfully! You can now refresh your application.' as result,
       'All tables, policies, and triggers have been created properly.' as details; 