-- ============================================================================
-- Ensure Transaction Tracking Functions Exist
-- This migration ensures the tracking functions are available, even if the main migration failed
-- ============================================================================
...
BEGIN
    ...
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'payment_sessions') THEN
    ...
    -- Create transaction_details table if it doesn't exist
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'transaction_details') THEN
    ...
    -- Create bank_test_cases table if it doesn't exist
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'bank_test_cases') THEN
...
-- Create or replace the tracking functions
...
-- Create or replace the transaction response function
...
-- Create or replace the security event logging function
...
-- Insert sample test cases if they don't exist
...
DO $$
BEGIN
    RAISE NOTICE 'Transaction tracking functions ensured successfully!';
    RAISE NOTICE 'Functions available: create_tracked_payment_session, record_transaction_response, log_security_event';
END $$; 